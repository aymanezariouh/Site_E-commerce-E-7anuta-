    ...
    <div class="py-10 dashboard-bg">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <section class="dash-card px-6 py-6 sm:px-8">
                <h2 class="dash-title text-2xl text-slate-800">Modifier la categorie</h2>
                <p class="text-sm text-slate-600 mt-1">Mettez a jour les informations.</p>
            </section>

            <div class="dash-card px-6 py-6 sm:px-8">
                <form method="POST" action="{{ route('seller.categories.update', $category) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-teal-500 focus:ring-teal-500" rows="4">{{ old('description', $category->description) }}</textarea>
                        <x-input-error :messages="$errors->get('description')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="image" value="Image (URL)" />
                        <x-text-input id="image" name="image" type="text" class="mt-1 block w-full" value="{{ old('image', $category->image) }}" />
                        <x-input-error :messages="$errors->get('image')" class="mt-2" />
                    </div>

                    <div class="flex items-center gap-2">
                        <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-slate-300 text-teal-600 shadow-sm focus:ring-teal-500" @checked(old('is_active', $category->is_active))>
